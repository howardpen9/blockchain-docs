(function (global, factory) {
    ...
    async function getFiles(dir, route) {
      ...
      const items = (await Promise.all(files.map(async f => {
        ...
        if (f.isDirectory()) {
          ...
          const meta = JSON.parse(await fs.promises.readFile(filePath, 'utf-8'));
  ...
  async function loader (source) {
    ...
    const {
      ...
      locales,
      defaultLocale
    } = options; // Add the entire directory `pages` as the dependency
    // so we can generate the correct page map

    this.addContextDependency(path.resolve('pages')); // Generate the page map

    let [pageMap, route] = await getPageMap(this.resourcePath); // Extract frontMatter information if it exists
